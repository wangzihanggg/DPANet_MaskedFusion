...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num_pt, add_noise, root, noise_trans, refine):
        if mode == 'train':
            ...
            self.path = 'datasets/ycb/dataset_config/new_train_data_list.txt'
        elif mode == 'test':
            # self.path = 'datasets/ycb/dataset_config/test_data_list.txt'
            self.path = 'datasets/ycb/dataset_config/new_test_data_list.txt'
        self.num_pt = num_pt
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        # label = np.array(Image.open('{0}/{1}-pred_mask.png'.format(self.root, self.list[index])))
        label = np.resize(label,(480,640))
        ...
        if self.add_noise:
            for k in range(5):
                ...
                f_label = np.resize(f_label, (480, 640))

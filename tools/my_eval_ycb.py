...
parser.add_argument('--dataset_root', type=str, default='/project/1_2301/DPANet-master/YCB_Video_Dataset',
                    help='dataset root dir')
parser.add_argument('--model', type=str,
                    default='/project/1_2301/MaskedFusion-master/trained_models/ycb/pose_model_48_0.02942494787275791.pth',
                    help='resume PoseNet model')
parser.add_argument('--refine_model', type=str,
                    default='/project/1_2301/MaskedFusion-master/trained_models/ycb/pose_refine_model_1_0.5533661097288132.pth',
...
dataset_config_dir = '/project/1_2301/MaskedFusion-master/datasets/ycb/dataset_config'
ycb_toolbox_dir = '/project/1_2301/MaskedFusion-master/experiments/scripts/YCB_Video_toolbox'
result_wo_refine_dir = '/project/1_2301/MaskedFusion-master/experiments/eval_result/ycb/Densefusion_wo_refine_result'
if not os.path.exists(result_wo_refine_dir):
    os.mkdir(result_wo_refine_dir)
result_refine_dir = '/project/1_2301/MaskedFusion-master/experiments/eval_result/ycb/Densefusion_iterative_result'
if not os.path.exists(result_refine_dir):
    os.mkdir(result_refine_dir)
...
input_file = open('{0}/new_test_data_list.txt'.format(dataset_config_dir))
while 1:
...
for now in range(0, 5):
    ...
    image = cv2.imread('{0}/{1}-color.png'.format(opt.dataset_root, testlist[now])).astype(np.float32)[:, :, ::-1]
    my_depth = cv2.imread('{0}/{1}-depth.png'.format(opt.dataset_root, testlist[now])).astype(np.float32)[:, :, ::-1]
    ...
    label = np.array(execSS(image, my_depth))

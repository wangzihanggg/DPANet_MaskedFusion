...
parser.add_argument('--dataset_root', default='/project/1_2301/DPANet-master/YCB_Video_Dataset', help="dataset root dir (''YCB_Video Dataset'')")
...
if __name__ == '__main__':
    ...
    dataset = SegDataset(opt.dataset_root, '{0}/{1}'.format(opt.dataset_root, 'dataset_config/new_train_data_list.txt'), True, 20)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=opt.batch_size, shuffle=True, num_workers=int(opt.workers))
    test_dataset = SegDataset(opt.dataset_root, '{0}/{1}'.format(opt.dataset_root, 'dataset_config/new_test_data_list.txt'), False, 10)
    test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=1, shuffle=True, num_workers=int(opt.workers))
